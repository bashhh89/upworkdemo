...
import { CycleItem } from "./cycle-item"; // Import the component created above
import { CYCLE_ITEM_ELEMENT, CYCLE_ELEMENT } from "../lib"; // Import constants from lib.ts

export interface CycleElementProps extends TElement { // Use different name from component if needed
  type: typeof CYCLE_ELEMENT;
}

// Main cycle component with withRef pattern
export const CycleElement = withRef<typeof PresentationElement>( // Use correct base element type
  ({ element, children, className, ...props }, ref) => {
    const childrenArray = React.Children.toArray(children as ReactNode);
    const items = element.children; // These should be CycleItem elements
    ...
    return (
      ...
      >
        ...
        <div className="mx-auto grid grid-cols-3 gap-4 px-12 max-w-4xl"> {/* Added max-width */}
          {/* Central SVG wheel */}
          <div className="relative col-start-2 row-span-2 row-start-2 mx-auto h-64 w-64 flex items-center justify-center">
            <svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" className="absolute inset-0 h-full w-full fill-current text-blue-500/20">
          ...
          {childrenArray.map((child, index) => {
            ...
            if (hasOddItems && index === 0) {
              columnStart = "col-start-2"; // First item middle if odd total
            } else {
              const adjustedIndex = hasOddItems ? index - 1 : index;
              columnStart = adjustedIndex % 2 === 0 ? "col-start-1" : "col-start-3"; // Alternate left/right
            }

            return (
              <div key={index} className={cn("col-span-1", columnStart)}>
                {/* Pass index, totalItems etc to CycleItem */}
                <CycleItem
                  index={index}
...
CycleElement.displayName = 'CycleElement';


// Define Plate plugins for Cycle and CycleItem
...
export const CycleItemPlugin = createPlatePlugin({
  ...
  node: { isElement: true }, // Component association happens via CycleElement rendering CycleItem
}); 