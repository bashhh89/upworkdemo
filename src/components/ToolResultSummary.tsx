...
export default function ToolResultSummary({ result, onAskAbout }: ToolResultSummaryProps) {
  ...
  const getResultSubject = () => {
    ...
    } else if (result.toolName === 'Contextual Deal Writer' && result.parameters?.company) {
      return result.parameters.company;
    }
  ...
  const getToolIcon = () => {
    ...
    if (result.toolName.includes('Deal') || result.toolName.includes('Proposal')) return <Target className="h-5 w-5 text-green-500" />;
  ...
  const getToolBgClass = () => {
    ...
    if (result.toolName.includes('Deal') || result.toolName.includes('Proposal')) return 'from-green-900/20 to-green-600/10 border-green-800/40';
