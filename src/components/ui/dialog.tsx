...
export function DialogTrigger({ children, className, ...props }: React.HTMLAttributes<HTMLButtonElement>) {
  return (
    <button {...props} className={`focus:outline-none ${className}`}>
      {children}
    </button>
  );
}
...
export function DialogFooter({ children, className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div {...props} className={`mt-6 flex justify-end gap-3 ${className}`}>
      {children}
    </div>
  );
}
